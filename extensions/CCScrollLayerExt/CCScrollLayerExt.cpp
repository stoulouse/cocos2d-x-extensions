...
CCScrollLayerExt::CCScrollLayerExt(CCRect mRect)
{
	...
	contentLayer = CCLayerColor::layerWithColorWidthHeight(ccc4f(128, 128, 128, 0), mRect.size.width, mRect.size.height);
	...
	if (verticalScrollIndicator == NULL)
	{
		...
		verticalScrollIndicator = CCLayerColor::layerWithColorWidthHeight(ccc4f(50, 50, 50, 180), vSize.width,vSize.height);
	...
	if (horizontalScrollIndicator == NULL)
	{
		...
		horizontalScrollIndicator = CCLayerColor::layerWithColorWidthHeight(ccc4f(50, 50, 50, 180), hSize.width,hSize.height);
    ...
    this->schedule(schedule_selector(CCScrollLayerExt::updateIndicators));
}
...
{
//    this->unschedule(schedule_selector(CCScrollLayerExt::updateIndicators));
	contentLayer->stopAllActions();
...
void CCScrollLayerExt::updateIndicators(ccTime mTime)
{
...
void CCScrollLayerExt::ccTouchEnded(CCTouch *touch, CCEvent* event)
{
	...
	if (touch == beginTouch) {
		...
		if (speed >= 0.5f) { //速度
			...
			CCEaseOut * fadeOut = CCEaseOut::actionWithAction(CCMoveBy::actionWithDuration(0.3, deltaPos ),3);
			CCCallFunc * callBack = CCCallFunc::actionWithTarget(this, callfunc_selector(CCScrollLayerExt::constraintContent));
			contentLayer->runAction(CCSequence::actionOneTwo(fadeOut, callBack));
