...
SlidingMenuGrid* SlidingMenuGrid::menuWithArray(CCMenuItemsArray* items, int cols, int rows, CCPoint pos , CCPoint pad)
{ 
	return menuWithArray(items,cols,rows,pos,pad,false);
}


SlidingMenuGrid* SlidingMenuGrid::menuWithArray(CCMenuItemsArray * items, int cols, int rows, CCPoint pos , CCPoint pad, bool vertical)
{
...
bool SlidingMenuGrid::initWithArray(CCMenuItemsArray * items, int cols, int rows, CCPoint pos , CCPoint pad, bool vertical)
{
	if( !CCLayer::init() )
	{
		return false;
...
void SlidingMenuGrid::ccTouchEnded(CCTouch* touch, CCEvent* event)
{
	if( bMoving )
	{
		...
		{			
			...
			if( iPageCount > 1 && (fMoveDeadZone < abs((int)fMoveDelta)))
			{
